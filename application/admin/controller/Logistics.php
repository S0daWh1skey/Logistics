...
use think\Db;
class Logistics extends CommonController
{
    public function index(){
        if(request()->isPost()){
            $SearchItem = input('param.search');//获得传过来的值
            $doctors=Db::name('log1')->where('DIsgreen','like','%'.$SearchItem.'%')
                ->paginate(20);//根据姓名模糊搜索
            $this->assign("llist",$doctors);//将查询结果转化为模版
            return $this->fetch(); //显示界面
        }//搜索时用到改分支
        $doctors=Db::name("log1")->paginate(20);//查询数据分页，每页20个数据
        $this->assign("llist",$doctors);//将查询结果转化为模版
    ...
    public function delete($id){
        ...
        if(Db::name("log1")->delete($id))//执行删除sql
        {
    ...
    public function update(){
        if(request()->isPost()){
            $data=[
                "Logistics-Note"=>input('param.Logistics-Note'),
                "Logistics-AMOT"=>input('param.Logistics-AMOT'),
                "Logistics-Prescription"=>input('param.Logistics-Prescription'),
                "Logistics-Type"=>input('param.Logistics-Type'),
                "Logistics-Number"=>input('param.Logistics-Number'),
                "Logistics-Isgreen"=>input('param.Logistics-Isgreen'),
                "Logistics-Isreceive"=>input('param.Logistics-Isreceive'),
                "Logistics-Abnormal"=>input('param.Logistics-Abnorma'),
                "logisCompany"=>input('param.logisCompany'),
                "Lid"=>input('param.Lid'),
//              "Note"=>input('param.Note'),
//              "Mail"=>input('param.Mail'),
//              "City"=>input('param.City'),
//              "Area"=>input('param.Area'),
//              "IsCooperation"=>input('param.IsCoop'),
...
//          }//判断医生是否存在或重名
//          $data=[
//              "DName"=>input('param.DName'),//"数据库字段"=>input('param.Name') <input name="Name" id="" class=""></input>
//              "DPhone"=>input('param.DPhone'),
//              "DDpartment"=>input('param.DDept'),
//              "HName"=>input('param.Hname'),
//              "logisName"=>input('param.LogName'),
//              "logisPhone"=>input('param.LogPhone'),
//              "logisWay"=>input('param.LogWay'),
//              "logisCompany"=>input('param.LogCompany'),
//              "GetTime"=>input('param.Gettime'),
//              "Note"=>input('param.Note'),
//              "Mail"=>input('param.Mail'),
//              "City"=>input('param.City'),
//              "Area"=>input('param.Area'),
...
//  }//添加函数
    ...
    public function detail(){
        $Did=input('param.Logistics-Number');//获得Did
        //select用于查询0到的多条记录
        //find用于查询一条记录
        ...
        if($file){
            ...
            if($info){
                ...
                for($j=2;$j<=$highestRow;$j++){
                    ...
                    $sql = "INSERT INTO Doctor (Logistics-Note,Logistics-AMOT,Logistics-Prescription,Logistics-Type,Logistics-Number,Logistics-Isgreen,Logistics-Isreceive,Logistics-Abnorma,
Lid) VALUES('".$strs[0]."','".$strs[1]."','".$strs[2]."','".$strs[3].
