...
class Data extends CommonController
{
    ...
    public function index(){
        ...
        $doctors=Db::name("Doctor")->paginate(20);//查询数据分页，每页20个数据
        $this->assign("dlist",$doctors);//将查询结果转化为模版
    ...
    public function delete($id){
        ...
        if(Db::name("Doctor")->delete($id))//执行删除sql
        {
    ...
    public function update(){
        if(request()->isPost()){
            ...
            $res=Db::name("Doctor")->update($data);//更新sql语句
            if($res){
                echo "<script>
                        history.back(-1);
                        alert('修改医生信息成功');
...
//                        $(#my-popups).modal('hide');
    ...
    public function detail(){
        $Did=input('param.DoctorID');//获得Did
        //select用于查询0到的多条记录
        //find用于查询一条记录
        $Doctor=Db::name("Doctor")->find($Did);//根据Did查询数据库
